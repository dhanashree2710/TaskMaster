-- ===========================================
-- PATCH: task file/photo attachments (optional, on task creation + detail)
-- ===========================================
-- Safe to re-run. Run this in the Supabase SQL editor.

-- Make sure task_files exists with the columns the app needs, and that
-- deleting a task cleans up its attached files automatically.
...
-- If task_id was created before the cascade was added, upgrade it so
-- deleting a task also removes its attachments instead of erroring.
do $$
begin
...
-- The task_files storage bucket needs to be public (like employee_photos,
-- wall_images, etc.) so the public URL returned after upload actually
-- resolves to the file — otherwise attachments upload fine but show as
-- broken links.
